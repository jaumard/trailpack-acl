...
module.exports = {
  ...
  loadFixtures: app => {
    //Waterline Compat Mode (Move to Waterline)
    if (app.config.database.orm === 'waterline'){
      if (app.orm.Role.bulkCreate === undefined){
        app.orm.Role.bulkCreate = app.orm.Role.create
      }
      if (app.orm.Resource.bulkCreate === undefined){
        app.orm.Resource.bulkCreate = app.orm.Resource.create
      }
      if (app.orm.Permission.bulkCreate === undefined){
        app.orm.Permission.bulkCreate = app.orm.Permission.create
      }
    }
  ...
  loadResources: app => {
    let resources = app.config.permissions.fixtures.resources
      .concat(app.packs.acl.routesFixtures.resources)
  ...
  loadPermissions: app => {
    const permissions = app.config.permissions.fixtures.permissions
      .concat(app.packs.acl.routesFixtures.permissions)
